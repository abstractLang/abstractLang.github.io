...
Ass everthing in abstract, you has full controll of every rsource provided, the language gives you full hability to manually choose when resources must be freeled. To disable the garbage collecting analysis and consequently, the implicit `delete` statements,use the `Std.GC` namespace to manuually control the garbage collector's behavior:
